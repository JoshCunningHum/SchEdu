<template>
    <UContainer class="flex flex-col gap-5 py-5 h-full w-full min-h-0">
        <PreLoader v-if="isOpeningTimeTable" message="Opening Timetable" />
        <div class="h-24 flex gap-5">
            <!-- <div
                class="w-52">
                <Logo calendar/>
            </div> -->
            <div class="flex flex-grow gap-2">
                <div class="flex-grow flex justify-center items-end">
                    <div class="text-2xl jetbrainsmono">Timetables</div>
                </div>
        ...
        <div class="flex flex-grow flex-col rounded-md p-4 min-h-0 gap-2">
                ...
                <div class="flex gap-2">
                    <template v-if="selectionMode">
                        ...
                        <UButton color="red" @click="bulkRemove()">
                            Delete {{ selectedFiltered.length }} Item{{
                                selectedFiltered.length > 1 ? "s" : ""
                            }}
                        </UButton>
                    </template>
            ...
            <div
                class="flex flex-grow flex-col gap-2 overflow-y-auto min-h-0"
                id="timetable-cont"
            >
                <!-- Timetables Here -->
                ...
                <template v-else>
                    <div
                        v-for="(t, index) in timeTablesFiltered"
                        class="roboto cursor-pointer"
                    >
                        <UContainer
                            class="flex-grow flex justify-between gap-2 py-2"
...
const selectedFiltered = computed(() =>
    timeTablesFiltered.value.filter((t) => isSelected(t.id)).map((t) => t.id)
);
...
const bulkRemove = () => {
    router.push({
        path: "/dashboard/remove-multiple",
        query: { ids: JSON.stringify(selectedFiltered.value) },
    });
};
...
../controllers/useDashboardControl